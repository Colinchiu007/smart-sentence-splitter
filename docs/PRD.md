...
### 句子层 / 场景层边界规则（v0.13.0 沉淀）

历轮质量优化沉淀的分句边界规则，按「句子层（Layer 1）→ 场景层（Layer 2）」分组：

| 层级 | 规则 | 说明 | 引入版本 |
|------|------|------|---------|
| **句子层** | 换行 `\n` 作 EOS 边界 | `ChineseSplitter.EOS_CHARS` 含 `\n`，标题行与正文、短行叙述正确分离 | v0.13.0 |
| **句子层** | 引号闭合处 EOS | `_protect_quoted` 剥离引文尾部句末标点保留在占位符外，使 `。”` 处 EOS 检测可见 | v0.13.0 |
| **句子层** | TextTiling 句级窗口 | `sentence_window`（默认 3 句）滑动窗口余弦相似度，替代字符级窗口 | v0.9.10 / v0.13.0 属性统一 |
| **场景层** | 标题行无终止符自动补句号 | `_create_scene` 拼接多句时，前句无终止标点自动补 `。` 避免粘连 | v0.13.0 |
| **场景层** | 单句溢出控制 | `allow_single_sentence_overflow=True`：当前场景仅 1 句时允许继续追加，避免一句一场景碎片化 | v0.13.0 |
| **场景层** | 主题边界优先 | `is_topic_boundary` 强制切新场景，有边界时有效字数阈值自动 3x | v0.9.10 |
| **场景层** | 分隔线过滤 | `_is_separator_line` + `SeparatorLineProcessor` 过滤 `---`/`***`/`===` 纯分隔线段落 | v0.11.0 / v0.13.0 |
| **共享** | 短尾合并阈值 | `_resplit` 短尾合并 `< min_chars`（原 `<=4` 误合并引号保护 4 字尾块） | v0.13.0 |
| **共享** | 字幕短尾合并 | `_merge_short` `is_short_tail`：≤3 字短尾合并到前一块（前块 ≥ min_chars） | v0.13.0 |
| **共享** | 跨句不合并 | 前块末尾为句子终止标点（。！？）时不合并剩余文本，避免两句同屏 | v0.12.0 |

> 字幕层（Layer 3）的完整 7 步流水线、枚举保护、时间戳舍入等规则见 `docs/subtitle-segmentation-spec.md`（规范 v1.1，双实现共享）。
...
| **v0.10.1** | 2026-06-30 | 字幕分割质量修复 — 7 类问题（末尾标点/跨块引号/开头标点/扩大搜索/纯引号短块等） |
| **v0.11.0** | 2026-07-13 | 字幕分句规则优化 — 引号感知预分割、超长块强制分割、分隔线过滤 |
| **v0.12.0** | 2026-07-19 | 字幕跨句合并修复（短尾合并跨句边界保护 + 块内句号切分） |
| **v0.12.1** | 2026-08-11 | /v1/split 场景层透传字幕层（subtitles[] 契约） |
| **v0.13.0** | 2026-07-13 | 场景层/句子层规则修复 — 换行 EOS、标题补句号、场景溢出控制、短尾合并、引号闭合 EOS、TextTiling sentence_window、分隔线过滤（详见 CHANGELOG） |
| **v0.14.0** | 2026-08-11 | 字幕分割规则统一（规范 v1.0，方案 C：7 步流水线 + 双实现共享测试向量） |
| **v0.14.1** | 2026-08-12 | 字幕分块平衡切分 + 时间戳连续性（消除孤悬尾块与 0.01s 间隙） |
| **v0.14.2** | 2026-08-12 | Step 3 硬切尾块平衡 + 回归护栏（min_chars 不变量 / 严格连续性断言） |
| **v0.15.0** | 2026-08-12 | 顿号枚举单元整体保护（规范 v1.1，枚举切分 + 谓词引导词启发式） |
| **v0.15.1** | 2026-08-12 | 时间戳舍入统一 half-up（消除 Python/TS .xx5 边界分歧） |
| **v0.15.2** | 2026-08-12 | 字幕规则表单源 subtitle-rules.json（双实现共享，禁止手写硬编码） |
